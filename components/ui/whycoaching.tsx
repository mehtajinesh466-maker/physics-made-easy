...
export default function WhyChooseCoachingSection() {
  ...
  const benefits = [
    {
      ...
      description: 'We move beyond rote memorization. Students understand the "Why" and "How" behind every formula.',
      icon: <BrainCircuit className="w-6 h-6 md:w-8 md:h-8 text-amber-600" />, 
      bgClass: 'bg-amber-50', 
      accentClass: 'bg-amber-500', 
    },
    {
      title: 'Exam Strategy',
      description: 'Master time management, answer writing techniques, and board exam patterns.',
    ...
    },
    {
      ...
      description: 'Weekly chapter tests, cumulative assessments, and full-syllabus mock exams.',
      icon: <FileText className="w-6 h-6 md:w-8 md:h-8 text-emerald-600" />,
      bgClass: 'bg-emerald-50',
      accentClass: 'bg-emerald-500',
    },
    {
      title: 'Personal Mentorship',
      description: 'Small batch sizes ensure every student gets individual attention and doubt resolution.',
      icon: <Users className="w-6 h-6 md:w-8 md:h-8 text-rose-600" />,
      bgClass: 'bg-rose-50',
  ...
  return (
    <section className="py-16 md:py-24 px-4 sm:px-6 lg:px-8 relative overflow-hidden bg-slate-50 font-sans">
      ...
      {/* Background: Geometric/Math Pattern */}
      <div 
        ...
        <div className="flex flex-col md:flex-row flex-wrap justify-center items-center gap-6 md:gap-8 lg:gap-12 relative">
          ...
          <div className="w-full md:w-auto flex justify-center items-center order-1 md:order-2 group cursor-default mb-4 md:mb-0">
            <div className="relative">
              {/* Spinning Ring */}
              <div className="absolute inset-[-10px] border-2 border-dashed border-amber-300 rounded-full animate-[spin_12s_linear_infinite]" />
              ...
              <div className="absolute bottom-4 right-0 bg-white px-4 py-2 rounded-xl shadow-lg border border-amber-100 animate-bounce-slow flex items-center gap-2">
                 <CheckCircle2 size={18} className="text-amber-500 fill-amber-500" />
                 <span className="font-bold text-slate-800 text-xs uppercase tracking-wide">Proven Results</span>
              </div>
            </div>
          </div>
